...
import {
	type EnumValue,
	EnumValueSymbol,
	type SelectionSetSelection,
} from "./helpers";

export type AnyFragmentDefinition =
	// biome-ignore lint/suspicious/noExplicitAny: Many complex types that need these
	| FragmentDefinition<string, string, string, any>
	// biome-ignore lint/suspicious/noExplicitAny: Many complex types that need these
	| FragmentDefinitionWithVariables<string, string, string, any, any, any>;
...
type BuilderObject_Fragment = (
	fragment: AnyFragmentDefinition,
	variables?: Record<
		string,
		{
...
class FragmentMap extends Map<string, AnyFragmentDefinition> {
	fragments = new Set<AnyFragmentDefinition>();

	set(name: string, fragment: AnyFragmentDefinition) {
	...
	has(nameOrFragment: string | AnyFragmentDefinition) {
...
export class Field<
	...
	const Alias extends string | undefined = string | undefined,
	const Output = never,
> extends Selection {
	...
	alias<A extends string>(name: A): Field<Name, A, Output> {
		const clone = this as unknown as Field<Name, A, Output>;
		clone._alias = name;
		return clone;
	}
...
export class InlineFragment<
	const PossibleTypes extends string = string,
	const TypeCondition extends string = string,
	const _Output = never,
> extends Selection {
...
export class FragmentDefinition<
	...
	const PossibleTypes extends string,
	const TypeCondition extends string,
	const Output,
> extends Selection {
	public readonly _possibleTypes!: PossibleTypes;
...
export class FragmentDefinitionWithVariables<
	...
	const PossibleTypes extends string,
	const TypeCondition extends string,
	const Variables extends Record<string, unknown>,
	const Output,
	const VariableInput extends Record<string, unknown>,
> extends Selection {
...
export class FragmentSpread<F extends AnyFragmentDefinition> extends Selection {
...
export class Operation<const Output, const Variables> {
	constructor(
		...
		{},
		{
			get(_, prop) {
				...
				const builder: BuilderObject_Field = (arg1, arg2, arg3) => {
					let builder: Field_BuilderArg | undefined;
					let args: Field_ArgumentsArg | undefined;
					let _alias: Field_AliasArg | undefined;
...
function makeOperationBuilder(type: OperationTypeNode) {
	return function builderOperation(
	...
	): Operation<unknown, Record<string, unknown>> {
		...
		if (typeof arg1 === "object") {
			...
			builder = arg2 as Operation_BuilderArg;
...
function makeFragmentBuilder() {
	return (
		...
		if (typeof arg3 === "object") {
			...
			builder = arg4 as Fragment_BuilderArg;
		} else {
			builder = arg3;
		}
		if (variables) {
