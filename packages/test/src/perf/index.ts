...
async function measure(
	samples: number,
	fn: () => Promise<DocumentNode> | DocumentNode,
): Promise<{
	min: number;
	max: number;
	average: number;
...
async function runPerformanceTest(
	...
	console.log(
		`${label}: ${metrics.definitions} definition${metrics.definitions !== 1 ? "s" : ""}, ${metrics.fields} field${metrics.fields !== 1 ? "s" : ""}`,
	);
...
async function main() {
	const testFiles = await fs
		.readdir(__dirname)
		.then((files) => files.filter((file) => /\.perf\.(ts|js)$/.test(file)));
